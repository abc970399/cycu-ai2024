...
def check_files(day_beg, day_end):
    ...
    for day_cur in pd.date_range(day_beg, day_end):
        ...
        url = "https://tisvcloud.freeway.gov.tw/history/TDCS/M05A/" + str(day_cur_str) + ".tar.gz"
        print(url)
...
def download_and_extract_files(day_beg, day_end, download_dir):
    ...
    for day_cur in pd.date_range(start=day_beg, end=day_end):
        ...
        for step in pd.date_range(start=step_1st, end=step_end, freq=step_delta):
            ...
            csv_file_path = f"{download_dir}/M05A/{day_cur_str}/{hour_str}/TDCS_M05A_{day_cur_str}_{hour_str}{minutes_str}00.csv"
...
if __name__ == '__main__':
    ...
    # for each month
    month_beg = datetime.datetime(2024, 4, 30, 0, 0, 0)
    month_end = datetime.datetime(2024, 4, 30, 23, 55, 0)
